...
class BOExperiments:
    def __init__(self,
        ...
        self.res = pd.DataFrame(np.nan, index=np.arange(int(self.config['n_init']+ self.config['max_iters'] * self.config['batch_size'])),
    ...
    def get_path(save_path: str, antigen: str, kernel_type: str, seed: int, cdr_constraints: int, seq_len: int,
                 search_strategy: str, batch_size,
    ...
    def results(self, optim, x, itern, rtime):
        ...
        if Y[:(itern + 1)].shape[0]:
            ...
            # sequential
            if self.config['batch_size'] == 1 or len(optim.casmopolitan.fX) <= self.config['n_init']:
                argmin = np.argmin(Y[:(itern + 1) * 1])
                x_best = ''.join([self.f_obj.fbox.idx_to_AA[j] for j in
                                  optim.casmopolitan.X[:(itern + 1) * 1][argmin].flatten()])
                self.res.iloc[itern, :] = [itern, float(Y[-1]), float(np.min(Y[:(itern + 1)])), rtime,
                                           self.f_obj.idx_to_seq(x)[0], x_best]
            # batch
            else:
                actual_itern = itern - self.config['n_init']
                for idx, j in enumerate(
                        range(self.config['n_init'] + actual_itern * self.config['batch_size'],
                              self.config['n_init'] + (actual_itern + 1) * self.config['batch_size'])):
    ...
    def run(self):
        ...
        for itern in range(self.start_itern, self.config['max_iters'] + self.config['n_init']):
            start = time.time()

            if len(optim.casmopolitan.fX) < self.config['n_init']:
                x_next = optim.suggest(1)
            else:
            ...
            if itern % 2 == 0:
                if len(optim.casmopolitan.fX) < self.config['n_init']:
                    self.log(f"Random sample {itern + 1} / {self.config['n_init']} in {end - start:.2f} s")
                else:
                    self.log(f"Iter {itern + 1 - self.config['n_init']} / {self.config['max_iters']} in {end - start:.2f} s")
            self.save(optim)
        end = time.time()
...
if __name__ == '__main__':
    ...
    for antigen in antigens:
        ...
        seeds = list(range(args.seed, args.seed + args.n_trials)) #random seeds (I think)
        t = args.resume_trial
