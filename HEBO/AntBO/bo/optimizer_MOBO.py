...
class Optimizer:
    ...
    def suggest(self, n_suggestions=1):
        if self.batch_size is None:  # Oliver edit: Remember the batch size on the first call to suggest   old version: # restart the optimizer and set the first batch size
            ...
            self.restart()
        else:  # Oliver edit: set the batch size each time so that we can the batch size when we are done with our random sampling (don't restart here)
            self.batch_size = n_suggestions
            self.casmopolitan.batch_size = n_suggestions
